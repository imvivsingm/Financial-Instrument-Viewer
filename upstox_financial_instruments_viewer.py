...
def main():
    ...
    if uploaded_file is not None:
        ...
        with col3:
            unique_exchanges = df['exchange'].nunique() if 'exchange' in df.columns and not df['exchange'].isna().all() else 0
            st.metric("Exchanges", unique_exchanges)
        with col4:
            unique_types = df['instrument_type'].nunique() if 'instrument_type' in df.columns and not df['instrument_type'].isna().all() else 0
        ...
        if 'lot_size' in df.columns:
            ...
            if not lot_size_values.empty and len(lot_size_values.unique()) > 1:
                ...
                if min_lot != max_lot:
                    ...
                    df = df[(df_lot_numeric >= lot_range[0]) & (df_lot_numeric <= lot_range[1]) & df_lot_numeric.notna()]
        ...
        if 'strike_price' in df.columns:
            ...
            if not strike_price_values.empty and len(strike_price_values.unique()) > 1:
                ...
                if min_strike != max_strike:
                    ...
                    df = df[(df_strike_numeric >= strike_range[0]) & (df_strike_numeric <= strike_range[1]) & df_strike_numeric.notna()]
        ...
        if len(df) > 0:
            ...
            display_columns = []
            available_columns = [
                'name', 'trading_symbol', 'instrument_type', 'exchange', 'segment',
                'underlying_symbol', 'underlying_type', 'expiry_date', 'strike_price_formatted',
                'lot_size', 'minimum_lot', 'tick_size', 'weekly'
            ]
            ...
            with col2:
                if 'lot_size' in df.columns:
                    ...
                    st.write(f"**Average Lot Size:** {df['lot_size'].mean():.0f}")
                    st.write(f"**Median Lot Size:** {df['lot_size'].median():.0f}")
                    st.write(f"**Min Lot Size:** {df['lot_size'].min():.0f}")
                    st.write(f"**Max Lot Size:** {df['lot_size'].max():.0f}")
